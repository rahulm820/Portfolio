...
const DESC_CLAMP = 180; // chars beyond which a description is collapsed behind "Read more"
...
export default function Projects() {
  ...
  // Filter → then sort: featured first, then projects with a GitHub link, stable otherwise.
  const ordered = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      if (feat) return feat;
      return (b.github ? 1 : 0) - (a.github ? 1 : 0);
    });
  }, [activeFilter]);

  const slides = useMemo(() => chunk(ordered, PER_SLIDE), [ordered]);
  const slideCount = slides.length;

  // Reset to first slide whenever the filter changes.
  useEffect(() => { setSlide(0); }, [activeFilter]);

  // Auto-advance (paused on hover / when only one slide).
  useEffect(() => {
    if (paused || slideCount <= 1) return;
    const id = setInterval(() => setSlide(s => (s + 1) % slideCount), AUTOPLAY_MS);
    return () => clearInterval(id);
  }, [paused, slideCount]);

  const go = (i) => setSlide((i + slideCount) % slideCount);
      ...
      <div className="container">
        ...
        >
          <div className="carousel-viewport">
            ...
            >
              {slides.map((group, gi) => (
                <div className="carousel-slide" key={gi} aria-hidden={gi !== slide}>
                  {group.map(project => (
                    <article
                      key={project.id}
                    ...
                    >
                      ...
                      {(() => {
                        const isLong = project.description.length > DESC_CLAMP;
                        const isOpen = !!expanded[project.id];
                        return (
                          <div className="project-desc-wrap">
                            <p className={`project-desc${isLong && !isOpen ? ' clamped' : ''}`}>
                              {project.description}
                            </p>
                            {isLong && (
                              <button
          ...
          {slideCount > 1 && (
            <>
              <button className="carousel-arrow prev" onClick={() => go(slide - 1)} aria-label="Previous projects">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
              </button>
              <button className="carousel-arrow next" onClick={() => go(slide + 1)} aria-label="Next projects">
